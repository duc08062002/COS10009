...
class Food
  ...
  attr_accessor :x, :y, :type, :image, :vel_x, :vel_y, :angle, :x, :y, :score, :smoke
...
def draw_food food
  ...
  case 
  when ((Gosu.milliseconds/1000) %10 <7)
    food.image.draw_rot(food.x, food.y, ZOrder::FOOD, food.angle)
  when ((Gosu.milliseconds/1000) %10 <10 && (Gosu.milliseconds/1000) %10 >= 7)
